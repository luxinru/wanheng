	...
	export default {
		...
		mounted() {
			...
			if (this.channel === 'alipay_app') {
				...
				axios.post("https://ls.hzyuechuang.com/api/user/recharge_type", {
					money: this.money,
					type: this.channel,
					...
					// 向ios传支付宝签名字符串
					window.webkit.messageHandlers.show.postMessage(String(res.data));
		...
		methods: {
			...
			alipayApp(m = 50) {
				...
				if (/LT-APP/.test(navigator.userAgent)) {
					// 获取可用
					var pay = host + '/api/user/recharge_type';
